...
class TPLinkMRClientBase(AbstractRouter):
                            ...
                            if self._ipv6_support:
                                ...
                                for intf_v6 in wan_aux_values:
                                    if intf_v6.get('name') == ipv6_intf:
                                        ...
                                        status._wan_ipv6_addr = get_ipv6(intf_v6.get('X_TP_ExternalIPv6Address', '::'))
                except:
                    self._wan_failover_support = False
    ...
    def set_wan_backup(self, enable: bool) -> None:
        ...
        acts = [
            self.ActItem(self.ActItem.SET, 'WAN_USB_3G_LINK_CFG', '{},1,0,0,0,0'.format(i), 
                attrs=['backupEnable={}'.format(int(enable))]),
        ]
        self.req_act(acts)
    ...
    def set_ewan_connect(self, enable: bool) -> None:
        ...
        _, values = self.req_act(acts)
        self._logger.info('set_ewan values: %s', values)
        i = 0
        for intf in self._to_list(values):
            i += 1
        ...
        self._logger.info('dhcp command: %s', dhcp_command)
        acts = [
